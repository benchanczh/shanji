...
- **第三层 外部来源（小红书等）= 链接回源（v0.3 细化）**：原则是**「事实进库，表达留在源头」**——版权保护表达（文案/图片/视频），不保护事实（菜名/食材/用量）。
  - 库里只存：结构化食材与用量（求解器/过敏过滤/买菜清单的刚需）+ 自己的话写的简要步骤 + `source_url` + 作者署名。
  - App 展示：菜卡带来源徽章「来源：小红书 @作者」+ **跳转原帖**按钮，用户点过去看最真实的图文/视频。**绝不热链原帖图片/视频**（既是版权红线，平台防盗链也会让图挂掉）；外部食谱不需要我们配图。
  - 导入方式：**用户主动粘贴链接**触发 AI 解析（走校验流水线 + pending_review），不做批量爬取。
  - 降级：原帖被删后结构化事实仍可排菜单/出清单，"查看原帖"置灰提示失效。
...
- 内置精选食谱库（初期 50–100 道常用家常菜，**双语文字步骤为必需，图片可选**，AI 辅助录入）。**组库原则（v0.2.5 修正）**：食谱库是**全局共享内容、多菜系组库**，不绑定任何单一家庭的偏好——每个支持的菜系保证最低主菜深度（≥14 道，覆盖两周不重复窗口）；"主菜系占比 60%"由求解器在**选菜时**按各家庭的 `primary_cuisine` 配置实现。首个家庭（用户本人）主菜系为粤菜，因此种子库粤菜最深，但架构上任何家庭可设任意主菜系（SaaS-ready）。
...
| # | 风险/问题 | 影响 | 建议 |
|---|---|---|---|
| R1 | 小红书等外部抓取的反爬与版权/合规边界 | 可用性与法律风险 | 「事实进库，表达留在源头」（v0.3，见 §9 第三层）：用户粘贴链接导入结构化事实 + 跳转回源看原帖；不热链图片/视频、不批量爬取 |
...
5. （v0.2.5 已答）用户家庭主菜系 = **粤菜**；但食谱库为全局多菜系内容，偏好属于 household 配置，任何家庭可设任意主菜系（见 §13.1 组库原则）。
